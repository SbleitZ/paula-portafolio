...
const contacts = [
  {
    ...
    html: (
      <>
        Podés ver mi portafolio en{" "}
        <Link className="flex justify-center" href="#">
          <Underline text="Linkedin" gradiant="bg-linear-verde" />
        </Link>
      </>
    ),
  },
  ...
  },
  {
    ...
    html: (
      <>
        ...
        <Link className="flex justify-center" href="#">
          <Underline text="Behance" gradiant="bg-linear-violet-azul" />
        </Link>
      </>
    ),
  },
...
function ContactBox({ icon, html }: ContactBoxProps) {
  return (
    <div className="flex flex-col p-10 lg:p-20 gap-2 text-paragraph font-bold justify-center items-center text-mb-paragraphText lg:text-paragraphText">
      {icon}
      <p className="text-center max-w-md">{html}</p>
    </div>
  );
}
